...
describe('Quad solution', () => {
  ...
  test('empty joins with variable predicate quad', () => {
  ...
  test('with variable joins with same variable value', () => {
    const pattern = quad(variable('id'), namedNode('fl:#name'), literal('Fred'));
    ...
    const solution = QuadSolution.EMPTY.join(pattern, actual1)?.join(pattern, actual2);
  ...
  test('with variable does not join with different variable value', () => {
    const pattern = quad(variable('id'), namedNode('fl:#name'), literal('Fred'));
    const actual1 = quad(namedNode('fl:fred'), namedNode('fl:#name'), literal('Fred'));
  ...
  test('with variable joins with same variable value in different position', () => {
    ...
    const solution = QuadSolution.EMPTY.join(pattern1, actual1)?.join(pattern2, actual2);
    expect(solution && solution.quads.length).toBe(2);
    expect(solution && solution.quads[0].equals(actual1)).toBe(true);
    expect(solution && solution.quads[1].equals(actual2)).toBe(true);
  ...
  test('with variable joins with different variable in same quad', () => {
  ...
  test('with variable joins with different variable in another quad', () => {
    const pattern1 = quad(variable('id1'), namedNode('fl:#name'), literal('Fred'));
